...
class ModelManager(object):
    ...
    def download_model(self, model_name, custom_output):
        ...
        if custom_output is None:
            ...
            sufix = os.path.split(custom_output)[1]
            if sufix == "tts":
                output_path = os.path.join(custom_output ,model_full_name)
            else:
                output_path = os.path.join(os.path.join(custom_output, "tts") ,model_full_name)
        if os.path.exists(output_path):
